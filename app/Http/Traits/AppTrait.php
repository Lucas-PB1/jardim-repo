...
trait AppTrait
{
    ...
    function toUsData($valor)
    {
        $dateObj = DateTime::createFromFormat('d/m/Y', $valor);
    ...
    {
        // Se for uma expressão raw, apenas retorne
        if ($column instanceof Expression) {
            return $column;
        }

        if (strpos($column, ' as ') !== false) {
            [$col, $alias] = explode(' as ', $column);
            return "$col as $alias";
        }
        return $column;
    }

    protected function formatColumns(array $columns)
    {
    ...
    public function assignPermissionsToRole($roleName, $modules)
    {
        $permissions = is_array($modules) ? $this->getPermissionsForModules($modules) : $modules;
        Role::firstOrCreate(['name' => $roleName])->syncPermissions($permissions);
    }

    public function getPermissionsForModules(array $moduleNames)
    {
        return Permission::whereIn(
            'name',
            collect($moduleNames)
                ->flatMap(fn($module) => ["create_{$module}", "read_{$module}", "update_{$module}", "delete_{$module}"])
        )->get();
    }
